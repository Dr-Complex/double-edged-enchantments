...
public class XP_Needle extends Item {
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
            ...
            if (!user.isSneaking()){
                if(itemStack.get(DEE_DataComponentTypes.XP_CONTAINER) != null ){
                    if(user.experienceLevel >= 1){
                        if(itemStack.get(DEE_DataComponentTypes.XP_CONTAINER) < 64){
                            this.increased = 1;
        ...
        if(itemStack.get(DEE_DataComponentTypes.XP_CONTAINER) != null){
            itemStack.setDamage(64 - (itemStack.get(DEE_DataComponentTypes.XP_CONTAINER)));
        }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        if(stack.get(DEE_DataComponentTypes.XP_CONTAINER) != null){
            tooltip.add(Text.of(stack.get(DEE_DataComponentTypes.XP_CONTAINER) + " / 64 "));
        }
